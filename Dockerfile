# ── Stage 1: build / install dependencies ─────────────────────────────────────
FROM python:3.12-slim AS builder

WORKDIR /build

# Install build tools
RUN apt-get update && apt-get install -y --no-install-recommends \
        gcc \
        libffi-dev \
        libssl-dev \
    && rm -rf /var/lib/apt/lists/*

COPY requirements.txt .
RUN pip install --no-cache-dir --prefix=/install -r requirements.txt


# ── Stage 2: runtime ──────────────────────────────────────────────────────────
FROM python:3.12-slim AS runtime

LABEL org.opencontainers.image.title="ssh-mosh-wan-optimizer"
LABEL org.opencontainers.image.description="SSH ↔ MOSH WAN Gateway"
LABEL org.opencontainers.image.source="https://github.com/siwatinc/ssh-mosh-wan-optimizer"

# mosh-client + mosh-server (in case remote is same host),
# openssh-client (for bootstrap SSH),
# libssl (runtime dep of cryptography)
RUN apt-get update && apt-get install -y --no-install-recommends \
        mosh \
        openssh-client \
        libssl3 \
    && rm -rf /var/lib/apt/lists/*

# Copy installed Python packages from builder
COPY --from=builder /install /usr/local

WORKDIR /app
COPY gateway/ gateway/
COPY main.py .

# Default key/config locations (can be bind-mounted)
RUN mkdir -p /etc/gateway /keys && chmod 700 /etc/gateway /keys

# Gateway SSH listen port
EXPOSE 2222/tcp

# Env defaults (override via docker run -e or docker-compose)
ENV GATEWAY_HOST=0.0.0.0 \
    GATEWAY_PORT=2222 \
    GATEWAY_HOST_KEY_PATH=/etc/gateway/host_key \
    GATEWAY_SSH_KEY_PATH=/keys/id_ed25519 \
    GATEWAY_AUTHORIZED_KEYS_PATH=/keys/authorized_keys

ENTRYPOINT ["python", "main.py"]
